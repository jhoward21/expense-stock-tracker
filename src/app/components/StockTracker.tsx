...
function StockTracker() {
  const [stocks, setStocks] = useState<any>({AAPL: null, NVDA: null, TSLA: null, AMZN: null, APP: null, PLTR: null, GOOGL: null}); // Setting the initial state of our values.
  const [lastUpdated, setLastUpdated] = useState<any>({AAPL: null, NVDA: null, TSLA: null, AMZN: null, APP: null, PLTR: null, GOOGL: null}); // Setting the state for current updates

  useEffect(() => {
    const stockSymbols = ["AAPL", "NVDA", "TSLA", "AMZN", "APP", "PLTR", "GOOGL"];
    ...
    stockSymbols.forEach((symbol) => {
      fetch(
        ...
        .then((stocks) => {
          ...
          setLastUpdated((prev: any) => ({...prev, [symbol]: stocks?.meta?.timestamp}));
        })
        .catch((error) => console.error(`Unable to grab ${symbol} stock data`, error)); // Error handling if unable to grab API data.
    });
  }, []);
        ...
        {Object.keys(stocks).map((symbol) => (
          stocks[symbol] && (
            <div key={symbol} className="stock-card">
              ...
              <button className="btn btn-primary" onClick={() => refreshStock(symbol)}>
                Refresh {symbol} Data
              </button>
            </div>
          )
